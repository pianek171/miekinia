...
- `/10x-init` — scaffolds the `/context` skeleton (`changes/`, `archive/`, `foundation/`) plus universal READMEs in each. `/10x-shape` requires `context/foundation/` to exist; if absent, it delegates to `/10x-init` (Step 0 below).
...
- **Header:** "Init?"
- **Options:**
  - **Yes — run /10x-init (Recommended):** Scaffolds the /context skeleton (changes/, archive/, foundation/) with READMEs, then continues shaping.
  - **No — stop here:** Exit without changes. You'll need to initialize before shape can run.

On "Yes": invoke `/10x-init` using your AI coding assistant's skill-invocation mechanism (NOT through a shell command). When `/10x-init` returns, re-check the precondition; if it now passes, continue to Step 0.5. On "No": print "Stopping. Run `/10x-init` when ready, then re-invoke `/10x-shape`." and STOP.

Do not duplicate `/10x-init`'s scaffold logic. Delegation to the relevant skill is the correct path.
...
- **Header:** "Resume?"
- **Options:**
  - **Resume from Phase [next] (Recommended):** Pick up where the prior session left off. Completed phases are summarized, not replayed.
  - **Restart from scratch:** Archive the existing shape-notes.md to context/foundation/archive/ and start a new session.
...
- **Header:** "Context"
- **Options:**
  - **[Greenfield|Brownfield] — correct (Recommended):** [Auto-detected mode description]
  - **[Other mode] — override:** Switch to [other mode] instead.
...
2. **Surface 3–5 gray areas** as multi-select decisions when the user's first attempt has ambiguities. Ask the user. Each option is a real position with a tradeoff, not a placeholder. (GSD gray-area discovery.)
...
Then surface gray areas (ask the user 2–4 questions, **allow multiple selections for questions where multiple positions can co-exist**):
...
Ask the user with options drawn from the most common shapes:
...
Ask the user with three options:
...
Same question options as greenfield.
...
Ask the user about each FR with 2–4 options framed as plausible counter-arguments (drawn from the FR's domain — not generic). Always include a "No counter-argument; it stands as written" option as the LAST option (not first), so the question forces the user to consider the challenge before dismissing it.
...
Ask the user with the rule shapes above as multiple-selection options (plus "I want to add a rule — give me a moment to think" and "I'm building this as pure CRUD anyway — record it"). If the user picks a rule, return to the one-sentence prompt. If they accept the empty-CRUD label, record it as `# TODO: domain rule — see Open Questions` per the schema and add an entry to a running `## Open Questions` block in shape-notes.md.
...
After product framing is locked, run **one** Non-Goals multiple-selection round. The shape is a multiple-selection avoid-list — but aimed at *scope* avoids (capabilities the MVP won't build / change won't touch, quality dimensions it won't aim for), not technology avoids. Ask:
...
Ask the user with 3–5 options drawn from the user's domain — NOT generic. Examples (regenerate per project):
...
- **Header:** "Cross-check"
- **Options:**
  - **Address gaps now:** Re-enter the relevant phase to fill in missing elements. Recommended if multiple elements are missing.
  - **Accept and finish:** Proceed despite the gaps. They will be recorded as warnings in the checkpoint and surfaced in /10x-prd's Open Questions.
  - **Restart phase [N]:** Go back to a specific phase and rebuild from there.
